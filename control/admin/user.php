...
class control extends adminbase {
	...
	function onadd() {
		...
		$role = getgpc('addrole', 'P');
		$outid = getgpc('addoutid', 'P');
		$authType = getgpc('addauthtype', 'P');
		...
		$uid = $_ENV['user']->add_user($username, $password, $email, 0, '', '', '', $role, $outid, $authType);
	...
	function onls() {
		...
		if(!empty($_POST['addname']) && $this->submitcheck()) {
			...
			if(($status = $this->_check_username($username)) >= 0) {
				if(($status = $this->_check_email($email)) >= 0) {
 					$_ENV['user']->add_user($username, $password, $email, 0, '', '', '', $role, $outid, $authType);
					$status = 1;
	...
	function onedit() {
		...
		if($this->submitcheck()) {
			...
			$role = getgpc('role', 'P');
			$authType = getgpc('authtype', 'P');
			$outid = getgpc('outid', 'P');
			$sqladd = '';
